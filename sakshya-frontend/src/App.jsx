...
import { CourtVerificationCard } from './components/CourtVerificationCard';
import { ComplianceAccessPanel } from './components/ComplianceAccessPanel';
...
import { RestrictedTraceabilityPanel } from './components/RestrictedTraceabilityPanel';
import {
  EmptyState,
  HashChainVisualizer,
  HashChip,
...
const permissionLabels = {
  ...
  incidentReplay: 'open an incident replay',
  offlineSync: 'queue authorised offline metadata',
  admin: 'manage system administration',
  vault: 'open the forensic comparison vault',
  bsaCertificate: 'download the BSA Section 63 certificate',
  accessCopy: 'request a watermarked access copy',
  accessReceipt: 'download an access receipt',
  traceability: 'view restricted traceability',
};
  ...
  const workspace = authStatus === 'signed-in' && session?.user ? (
    ...
    >
      <Routes>
        ...
        <Route path="/document/:docId" element={<DocumentDetailPage documents={documents} onAddAction={addAction} onVerify={verifyDocument} onTamper={tamperDocument} permissions={permissions} user={session.user} apiOnline={apiOnline} />} />
        ...
        <Route path="/anomalies" element={<RoleGate user={session.user} permission="view"><AnomaliesPage documents={documents} permissions={permissions} apiOnline={apiOnline} /></RoleGate>} />
        <Route path="/reports" element={<RoleGate user={session.user} permission="report"><ReportsPage documents={documents} onReportGenerated={markReportGenerated} apiOnline={apiOnline} apiError={backendError} permissions={permissions} /></RoleGate>} />
...
function LoginPage({ backendStatus, backendError, error, busy, onLogin, onRetry }) {
  ...
  return <main className="auth-screen">
    <section className="auth-brand-panel">
      ...
      <div className="auth-brand-panel__copy"><SectionEyebrow icon={Fingerprint}>NATIONAL EVIDENCE REGISTER</SectionEyebrow><h1>SAKSHYA</h1><p>Digital Evidence Integrity Platform</p><div className="auth-rule" /><h2>Custody you can prove.</h2><span>Every upload, handover and verification is recorded as a cryptographically linked event for authorised review.</span></div>
...
function LandingPage({ documents, apiOnline, permissions = {} }) {
  ...
  return <div className="landing-page">
    ...
    <div className="landing-cta"><div><SectionEyebrow icon={Building2}>NATIONAL EVIDENCE REGISTER</SectionEyebrow><h2>Make every record defensible.</h2><p>SAKSHYA is a Smart India Hackathon 2026 prototype for SIH26190 · Ministry of Home Affairs.</p></div>{permissions.upload && <Link to="/upload" className="button button--light">Register evidence <FilePlus2 size={16} /></Link>}</div>
  </div>;
}
  ...
  return (
    <div className="dashboard-page">
      ...
      <section className="stats-grid command-stats">
        ...
        <StatsCard label="Open security alerts" value={loading ? '—' : String(openAlerts).padStart(2, '0')} helper={openAlerts ? 'Review before court submission' : permissions.incidents ? 'No active signals' : 'Safe signals visible; traceability restricted'} icon={ShieldAlert} tone={openAlerts ? 'amber' : 'slate'} trend={openAlerts ? 'Review' : 'Clear'} onClick={permissions.view ? () => navigate('/anomalies') : undefined} />
      ...
      <OfflineSyncPanel documents={documents} apiOnline={apiOnline} canQueue={Boolean(permissions.offlineSync)} />
    </div>
  );
}
...
function DocumentDetailPage({ documents, onAddAction, onVerify, onTamper, permissions = {}, user, apiOnline }) {
...
function VerifyPage({ documents, onVerify, apiOnline, apiError, permissions = {} }) {
  ...
  useEffect(() => {
    if (requestedCase) {
      const caseRecord = documents.find((item) => String(item.caseId) === requestedCase);
      if (caseRecord && caseRecord.docId !== selectedId) setSelectedId(caseRecord.docId);
    } else if (!selectedId && documents[0]?.docId) {
      setSelectedId(documents[0].docId);
    }
  }, [documents, requestedCase, selectedId]);
...
function AnomaliesPage({ documents, permissions = {}, apiOnline }) {
...
function ReportsPage({ documents, onReportGenerated, apiOnline, apiError, permissions = {} }) {
  ...
  async function downloadCourtBundle() {
    if (!reportId || !apiOnline) return;
    setBundleBusy(true);
    try {
      const { blob, filename } = await api.downloadCourtBundle(reportId);
      const url = URL.createObjectURL(blob);
      const anchor = documentCreateLink(url, filename);
      anchor.click();
      URL.revokeObjectURL(url);
      toast.success('Court case bundle downloaded from the live backend.');
    } catch (error) {
      const message = actionErrorMessage(error, 'Could not download the court case bundle.');
      setReportError(message);
      toast.error(message);
    } finally {
      setBundleBusy(false);
  ...
  async function downloadBsaCertificate() {
    if (!reportId || !apiOnline || !permissions.bsaCertificate) return;
    setBsaBusy(true);
    setReportError('');
    try {
      const result = await api.downloadBsa63Certificate(reportId);
      const url = URL.createObjectURL(result.blob);
      const anchor = documentCreateLink(url, result.filename);
      anchor.click();
      URL.revokeObjectURL(url);
      toast.success('BSA Section 63 certificate downloaded from the live backend.');
    } catch (error) {
      const message = actionErrorMessage(error, 'Could not download the BSA Section 63 certificate.');
      setReportError(message);
      toast.error(message);
    } finally {
      setBsaBusy(false);
