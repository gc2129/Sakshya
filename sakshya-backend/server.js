...
const labels = { UPLOADED: 'Evidence uploaded', VIEWED: 'Evidence viewed', TRANSFERRED: 'Custody transferred', EDITED: 'Evidence edited', COURT_ACCESSED: 'Court access granted', VERIFIED: 'Integrity verified' };
...
function createRecord({ id, file, metadata = {} }) {
  ...
  const record = { id, name: file?.originalname || clean(metadata.fileName, `${id}_evidence_record`), size: file?.size || digestSource.length, mimeType: file?.mimetype || clean(metadata.mimeType, 'application/octet-stream'), caseId: clean(metadata.caseId, 'CASE/UNASSIGNED'), description: clean(metadata.description, 'Registered evidence record.'), evidenceType: clean(metadata.evidenceType, 'Digital Document'), classification: clean(metadata.classification, 'Sensitive'), originalHash: hash(digestSource), currentHash: hash(digestSource), createdAt: new Date().toISOString(), custodyHolder: clean(metadata.officerName, 'Forensic Officer'), tamperBlockIndex: null, pendingTransfer: null, auditChain: [] };
...
app.post('/api/evidence/:id/transfer', (req, res) => { const record = getRecord(req, res); if (!record) return; const pending = record.pendingTransfer; if (!pending) return res.status(400).json({ error: 'Request a transfer OTP before transferring custody.' }); if (Date.now() > pending.expiresAt) { record.pendingTransfer = null; save(record); return res.status(400).json({ error: 'The transfer OTP has expired.' }); } if (clean(req.body?.recipient) !== pending.recipient || hash(clean(req.body?.otp)) !== pending.otpHash) return res.status(401).json({ error: 'Invalid transfer details or OTP.' }); const holder = record.custodyHolder; record.custodyHolder = pending.recipient; record.pendingTransfer = null; append(record, { action: 'TRANSFERRED', actor: holder, role: 'Custody Handover', details: `OTP-verified custody transfer to ${record.custodyHolder}.` }); save(record); res.json(publicRecord(record)); });
